...
```
Full model after variable selection:
```{r}
model1 <- lm(Life.expectancy ~ Year + infant.deaths + Alcohol + percentage.expenditure + BMI + under.five.deaths + Polio + Total.expenditure + Diphtheria + GDP + thinness..1.19.years + Income.composition.of.resources + Schooling, data=train)
summary(model1)
```
Trimmed model after removing non significant values:
```{r}
model1 <- lm(Life.expectancy ~ Year + Alcohol + Total.expenditure + Diphtheria + HIV.AIDS + Income.composition.of.resources + Schooling, data=train)
summary(model1)
```
...
plot(model1)
```
Assumptions do not look fantastic, but passable. Normality looks solid. All outliers have low Cook's D. Equality of variance seems to be the only concern.
